...
export function recommendedOrder(levels) {
  return levels
    ...
    .sort(
      (a, b) =>
        ['Leicht', 'Mittel', 'Schwer'].indexOf(levels[a].difficulty?.tier) -
          ['Leicht', 'Mittel', 'Schwer'].indexOf(levels[b].difficulty?.tier) ||
        (levels[a].order ?? a) - (levels[b].order ?? b) ||
        a - b,
    );
}
export function puzzleNumber(levels, index) {
  const tier = levels[index]?.difficulty?.tier;
  return (
    recommendedOrder(levels)
      .filter((i) => levels[i].difficulty?.tier === tier)
      .indexOf(index) + 1
  );
}
