...
export function variantPuzzle(
  ...
  if (generation === 2 && mode === 'dual') {
    ...
    for (let k = 0; k < size * 3; k++) {
      ...
      if (
        cells.length < Math.floor(size / 4) ||
        ![0, 1, 2, 3].some((d) => {
  ...
  if (mode === 'linked') {
    ...
    for (let i = 0; i < size; i += 2) groups.push(pairing.slice(i, i + 2));
  ...
  if (generation === 2) puzzle.generatorVersion = 2;
...
export function savedVariant(mode, saved) {
  const generation = saved.generatorVersion === 2 ? 2 : 1;
  const id =
    generation === 2
      ? `variant-v2-${mode}-${saved.n}-${saved.seed}`
      : undefined;
      ...
      try {
        ...
        if (validState(l, saved.session))
          data.free[mode] = {
            ...
            ...(saved.generatorVersion === 2 ? { generatorVersion: 2 } : {}),
          };
      } catch {}
    }
  if (
