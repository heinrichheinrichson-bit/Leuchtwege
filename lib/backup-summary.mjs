import { variantCatalog, variantKey, variantStatus } from './variants.mjs';
...
export function backupSummary(file, sliding) {
  ...
  return {
    variants: variantCatalog.filter(
      (l) =>
        data[variantKey]?.sessions?.[l.id] &&
        variantStatus(l, data[variantKey].sessions[l.id]).solved,
    ).length,
