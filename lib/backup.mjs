import { variantKey, restoreVariants } from './variants.mjs';
...
export const isDataKey = (key) =>
  fixed.includes(key) ||
  /^leuchtwege-daily-v2:\d{4}-\d{2}-\d{2}:(turn|slide|rotate|dual|path|linked):[012]$/.test(
    key,
  ) ||
...
export function validateBackup(text, levels, sliding) {
  ...
  for (const [key, raw] of Object.entries(file.data)) {
    ...
    if (key === variantKey) {
      requireValid(same(raw, restoreVariants(raw)));
    } else if (key === 'leuchtwege-v2') {
      ...
      const [prefix, day, mode, slot] = key.split(':');
      const checked = restoreDaily(
        raw,
        day,
        mode,
        prefix === 'leuchtwege-daily-v2' ? Number(slot) : undefined,
      );
