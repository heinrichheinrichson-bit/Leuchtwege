...
assert.equal(
  /[`"']Testhilfe[`"']/.test(page),
  testBuild,
  'Developer button must only exist in test build',
);
assert.equal(
  page.includes('Test: Nächster Schritt'),
  testBuild,
  'Direct step button must only exist in test build',
);
assert.equal(
  page.includes('Werbung simulieren · +1 Tipp'),
  testBuild,
  'Reward simulation must only be reachable in test build',
);
assert(
  page.includes('new Worker(`/_next/static/solve-help.worker-'),
  'Solver must use same-origin worker URL',
);
const code = readFileSync(
  dir + '/' + readdirSync(dir).find((n) => n.startsWith('solve-help.worker-')),
  'utf8',
);
...
try {
  const puzzles = [
    JSON.parse(readFileSync('lib/levels.json'))[20],
    JSON.parse(readFileSync('lib/sliding-levels.json'))[5],
  ];
...
console.log(
  'PASS: shipped solver worker and ' +
    (testBuild ? 'enabled' : 'disabled') +
    ' developer/reward simulation controls.',
);
